...
<chapter1>第1章：天命概論（約1400文字）
冒頭は「{first_name}さん、」で始める。以下5つの小見出しをすべて含め、それぞれ複数段落で詳しく書く：
「魂のテーマとこの世に生まれた根本理由」── 生年月日の背景（誕生日の意味・季節）と占術データから、なぜこの魂として生まれてきたのかを解き明かす
「数秘術の視点」── LifePathの数字が示す本質を詳しく
「四柱推命・九星気学の視点」── 年柱・月柱・日柱・九星気学の組み合わせが示す性質を詳しく
「西洋占星術の視点」── 星座が示す本質を詳しく
「オーラの色と波動の特徴」── 色・周波数帯・周囲に与える印象</chapter1>

<chapter2>第2章：才能と使命（約1600文字）
以下4つの小見出しをすべて含める：
...
<chapter3>第3章：時代文脈（約1400文字）
以下3つの小見出しをすべて含める：
「{y1}〜{y3}年の時代の大きな流れ」── AI・経済・社会変化の具体的トレンドとその人の天命の交差点
「なぜ今こそこの人の天命が輝くのか」── 占術データ（年齢・命式）と時代の一致を具体的に
「時代が求めているこの人の役割」</chapter3>

<chapter4>第4章：3年ロードマップ（約1800文字）
「{y1}年後半」「{y2}年」「{y3}年」の3つの見出しに分け、各年について
・その年の占術的テーマ（九星気学・干支等）
・具体的にやるべきこと（複数）
・避けるべきこと
・月収目標の目安
を詳しく書く。{y1}年後半は今日から始められる30日以内・3ヶ月以内のアクションを含める</chapter4>

<chapter5>第5章：今日からの行動（約1800文字）
仕事・キャリア／人間関係・コミュニティ／学び・スキルアップ／健康・エネルギー管理／お金・豊かさ
の5カテゴリすべてに小見出しを立て、各カテゴリ4〜5個・合計20個以上の具体的アクションを列挙する。各アクションに占術的根拠を1行添える</chapter5>"""
    msg = client.messages.create(
        model="claude-opus-4-8",
        max_tokens=8192,
        system=SYSTEM,
...
<chapter6>第6章：ラッキー風水＆パワースポット（約1600文字）
以下5つの小見出しをすべて含める：
「ラッキーカラーの使い方」── 3色、それぞれの使い方（服・小物等）
「国内パワースポット5箇所」── 具体的な地名と、なぜその人に合うのかの占術的理由を各2〜3文で
「海外パワースポット3箇所」── 同様に
「ラッキーアイテム・ラッキーナンバー」── 各アイテムの意味を詳しく
「吉方位・凶方位」── 【算出済み占術データ】の吉方位・凶方位をそのまま使用し、再計算しないこと。方位ごとの活用法を添える</chapter6>

<chapter7>第7章：人間関係と天命の仲間（約1400文字）
以下3つの小見出しをすべて含める：
「天命を加速させる相性」── 相性の良い生まれ年タイプ・星座タイプを具体的に
「出会うべきキーパーソン3タイプ」── 各タイプに名前を付け、詳しく説明
「注意すべき人間関係のパターン」── この人が陥りやすい関係性の癖と対処法</chapter7>

<chapter8>第8章：魂の課題と乗り越え方（約1400文字）
以下4つの小見出しをすべて含める：
「繰り返しやすいカルマのパターン」
「天命を妨げる3つのブロックと解除法」── ブロックごとに解除法をセットで
「過去世のテーマと今世での昇華方法」
「ネガティブサイクルから抜け出すワーク」── 具体的に今日から実践できるワークを2つ</chapter8>"""
    msg = client.messages.create(
        model="claude-opus-4-8",
        max_tokens=8192,
        system=SYSTEM,
...
<chapter9>第9章：月別・季節別の吉凶カレンダー（約1200文字）
{y1}年{m1}月から{y2}年12月までを対象に、季節・四半期ごとに小見出しを立てて
エネルギーが高まる時期・休息推奨の時期・新しいことを始めるベストタイミング・注意すべき時期と対処法を書く</chapter9>

<chapter10>第10章：天命宣言文＆メッセージ（約1200文字）
以下3つの小見出しをすべて含める：
「天命宣言文」── 毎朝声に出して読むアファメーション形式（5〜7行）
「守護存在からのスピリチュアルメッセージ」
「10年後のビジョンレター」── 10年後の自分から今の{first_name}さんへの手紙形式</chapter10>

<chapter11>第11章：SNS戦略＆プロデューサー鑑定（約1600文字）
以下4つの小見出しをすべて含める：
