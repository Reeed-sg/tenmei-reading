...
・語尾は「〜です」「〜ます」「〜なのです」で言い切る、力強く断定的な文体にしてください
・「〜ですよ」「〜ですね」のような柔らかい語りかけ表現は一切使わないでください
・「──」（ダッシュ）を効果的に使い、余韻や強調を演出してください
・「それはもう、〜していい時です」「〜という保証書のようなものです」のように、
  鑑定士としての確信と権威を感じさせる、詩的で格調高い言い切り型の表現を随所に使ってください
・占術的根拠を各所に簡潔に添えてください
・第1章は、冒頭の「あなたはこの世に○○という使命を携えて生まれてきた」という宣言の直後に、
  「まず一番大切なことから伝えます。あなたの九星・日柱は◯◯──◯◯を司る星です。今の◯◯（現在の悩み・苦しみ）
  は、終わりではなく、◯◯として甦る/花開く前兆なのです。」という一段落を必ず入れてください。辛い経験の詳細は
  その後に語り、先に「すでに再生・好転が始まっている」と断定してから根拠として辛い経験に触れる順番にしてください
・威厳と断定調は「〜しなさい」という命令形ではなく、「〜のです」「〜する時です」「〜が実現します」のように
  運命・必然を言い切る形で出してください。行動項目の箇条書きは「〜する」という辞書形（体言止めに近い言い切り）で書き、
  「〜してください」「〜しなさい」を連発しないでください
・「警告」「危険」「絶対に〜してはいけない」のような読み手を怖がらせる語は使わないでください。注意喚起は
  「〜な時です」「それが○○からの知らせなのです」のように、愛を保ったまま気づきを促す表現にしてください
・「〜が最大の消耗源です」「〜が最大の敵です」のような、冷たい診断・敵視するような言い切りも避けてください。
  同じ内容でも「〜を一番曇らせてしまっているのです」「〜が最も苦手とするものなのです」のように、
  威厳を保ちながらも愛のある言葉で伝えてください
・クライアントのアンケート回答に書かれていない事実を作り出さないでください。特に「死にたい」等の強い言葉や、
  年齢・身体条件から見て非現実的な断定的希望（出産・健康の完全回復など取り返しのつかないテーマ）を、
  根拠なく肯定・断言しないでください
・クライアントの家族（母・父など）に鑑定士視点の三人称で言及する際は「お母様」「お父様」のように敬称を使ってください
  （「母」「父」だと鑑定士自身の家族を指すように読めてしまうため使わないでください）
・行動を促す文の締めは、できる限り「〜できます」「〜が実現します」のように可能性を肯定する形にし、
  「〜すべきです」「〜ねばなりません」のような義務・強制のニュアンスで終わらせないでください
・レポート全体を通して、恐怖心や義務感ではなく、腹の底からじんわりと湧く覚悟と魂からの喜び──
  自分の使命・進むべき道が見通せた安心感と静かな喜びを軸にしてください。派手にはしゃぐ高揚感ではありません
